...
$.fn.show_tooltip = function(text, color) {
    ...
    $('#' + rid).parent().prev().css({'border-top-color': color});
}
...
$.fn.show_err_tooltip = function(text) {
    $(this).show_tooltip(text, 'rgba(255,0,0,0.8)')
}
...
$.fn.bt_validate = function() {
    ...
    $.bt_validate.form.find('input[data-validate],select[data-validate],textarea[data-validate]').blur(function() {

        var validate_params = $(this).attr('data-validate').split('|');
        ...
        if (field_result) {
            ...
            //Hiding tooltip - Valido! else Invalido!
    ...
    $.bt_validate.form.submit(function() {
        $.bt_validate.result = true;
        $.bt_validate.form.find('input[data-validate],select[data-validate],textarea[data-validate]').trigger('blur');
